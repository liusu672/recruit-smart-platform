...
describe('offer demo operations', () => {
  ...
  it('requires salary, entry date and work location before sending', () => {
    ...
    expect(() => sendDemoOffer(draft, { operatorId: 2, note: '' })).toThrow('请先填写有效月薪')
  })

  it('moves a reviewed draft to sent and records the HR action', () => {
    const draft = structuredClone(initialDemoOffers.find((item) => item.status === 'DRAFT')!)
    const sent = sendDemoOffer(
      draft,
      { operatorId: 2, note: '薪资与入职日期已完成复核。' },
      '2026-07-16T10:00:00',
    )
  ...
  it('requires an auditable reason when revoking a sent Offer', () => {
    const sent = structuredClone(initialDemoOffers.find((item) => item.status === 'SENT')!)

    expect(() => revokeDemoOffer(sent, { operatorId: 2, reason: '   ' })).toThrow(
      '撤回 Offer 必须填写原因',
    )

    const revoked = revokeDemoOffer(
      sent,
      { operatorId: 2, reason: '录用方案调整，需重新审批。' },
      '2026-07-16T11:00:00',
    )
  ...
  it.each(['ACCEPTED', 'REJECTED'] as const)(
    ...
    (status) => {
      ...
      expect(() => sendDemoOffer(offer, { operatorId: 2, note: '尝试重新发送' })).toThrow(
        '只有草稿 Offer 可以发送',
      )
      expect(() => revokeDemoOffer(offer, { operatorId: 2, reason: '尝试撤回候选人终态' })).toThrow(
        '只有已发送 Offer 可以撤回',
      )
    },
  )
