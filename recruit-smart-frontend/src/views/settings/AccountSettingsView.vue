...
const roleFallback = computed(
  () =>
    ({
      ...
      INTERVIEWER: '面试官',
      CANDIDATE: '候选人',
    })[session.currentRole ?? 'CANDIDATE'],
)
const description = computed(() =>
  session.currentRole === 'CANDIDATE'
    ? '管理登录账户信息和密码，求职资料请前往个人中心维护。'
    : '维护当前登录账号的联系方式和密码。',
)
...
<template>
  <div class="account-settings-view">
    <SupportPageHeader title="账户与安全" :description="description" />
    <SupportErrorState
    ...
    <section class="account-settings-section">
      ...
      <dl class="account-settings-facts">
        <div>
          <dt>登录账号</dt>
        ...
        <div>
          <dt>系统角色</dt>
          <dd>{{ userQuery.data.value?.roleName || roleFallback }}</dd>
        </div>
      </dl>
    ...
    <section class="account-settings-section account-settings-section--security">
      <header>
    ...
    >
      <el-form label-position="top" @submit.prevent="savePassword">
        ...
        /></el-form-item>
        <div class="account-password-strength">
          <span>输入强度参考：{{ passwordStrength.label }}</span>
          <div aria-hidden="true">
            <i
              v-for="level in 3"
              :key="level"
              :class="{ 'account-password-strength__active': passwordStrength.level >= level }"
            />
          </div>
...
.account-settings-view {
  display: grid;
  width: min(100%, var(--rs-support-settings-width));
  gap: var(--rs-space-4);
  margin: 0 auto;
}
...
.account-settings-section header p {
...
.account-settings-facts > div + div {
...
.account-password-strength > div {
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
  gap: var(--rs-space-1);
}
.account-password-strength i {
  height: 4px;
  border-radius: var(--rs-radius-xs);
  background: var(--rs-surface-muted);
}
.account-password-strength__active {
  background: var(--rs-action-primary) !important;
}
.account-password-strength small {
